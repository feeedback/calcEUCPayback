...
const fnOutputsMap = {
    ...
    mileagePerMonth: ({
        tripsToWorkPerMonth,
        distanceToWork,
        tripsToStorePerMonth,
        distanceToStore,
    }) =>
        (tripsToWorkPerMonth * distanceToWork + tripsToStorePerMonth * distanceToStore) *
        2,
    EUCCostsMonthly: ({
        mileagePerMonth,
        ...
        (
            ...
            (priceOfElectricityDay * shareOfDaytimeFromAll +
                priceOfElectricityNight * (1 - shareOfDaytimeFromAll))
        ).toFixed(1),
...
const calculateAndSetOutputValue = (input) => {
    ...
    const objInputsValues = () =>
        Object.fromEntries(
            Object.values(formEls).map((inputEl) => [inputEl.name, Number(inputEl.value)])
        );
...
document
    ...
    .addEventListener('input', calculateInputHandler);
