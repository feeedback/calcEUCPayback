...
const createOptionForDatalist = (input) => {
  const datalist = document.querySelector(`.custom-range-wrapper #${input.getAttribute('list')}`);
  ...
  const roundLabelStep = roundStep((max - min) / COUNT_LABELS_INPUT_RANGE_DATASET_OPTION);
...
const setFillPercentClassForInputRangeTrack = (input) => {
  ...
  const prevClassFillPercent = input.className.split(' ').find((cl) => cl.includes('fill_percent'));
...
const customInputRangeInit = () => {
  const inputsRangeList = document.querySelectorAll('.custom-range-wrapper input[type="range"]');
...
document
  ...
  .forEach((inputRange) => inputRange.addEventListener('input', customInputRangeHandler));
